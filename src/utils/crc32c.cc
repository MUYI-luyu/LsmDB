// CRC32C 的可移植实现（基于多项式 0x82F63B78）。
...
// 判断当前 CPU 是否可以加速 CRC32C 计算。
// 本可移植实现默认不支持硬件加速，始终返回 false。
static bool CanAccelerateCRC32C() {
  // 在此可移植版本中，暂不提供硬件 CRC32C 指令的检测与使用。
  // 如需硬件加速，请在下游移植层实现 port::AcceleratedCRC32C。
  return false;
}
...
uint32_t Extend(uint32_t crc, const char* data, size_t n) {
  ...
  if (accelerate) {
    // 硬件加速路径：由 port::AcceleratedCRC32C 提供
    // （当前可移植实现未启用）
    return 0;
  }
...
#define STEP4W(w)                                   \
  ...
  // 将指针 x 指向缓冲区中第一个 4 字节对齐的地址。此地址可能超过缓冲区末尾。
  ...
  if ((e - p) >= 16) {
    ...
    // 通过交错预取 256 字节提前量并一次处理 64 字节，
    // 可以在 x86 上获得更好的速度。参见 CRC32C 的可移植实现：
