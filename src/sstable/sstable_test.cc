...
class ReverseKeyComparator : public Comparator {
 public:
  const char* Name() const override {
    return "lsmdb.ReverseBytewiseComparator";
  }
...
class Harness : public testing::Test {
 public:
  ...
  // Returns nullptr not running against a DB
...
// 对没有重启条目的块进行特殊测试。C++ 版本的代码
// 永远不会生成这种块，但 Java 版本会。
// seems to.
...
TEST_F(Harness, RandomizedLongDB) {
  ...
  for (int level = 0; level < config::kNumLevels; level++) {
    std::string value;
    char name[100];
    std::snprintf(name, sizeof(name), "lsmdb.num-files-at-level%d", level);
...
#if defined(LSMDB_HAVE_SNAPPY) || defined(LSMDB_HAVE_ZSTD)
...
static bool CompressionSupported(CompressionType type) {
  ...
  if (type == kSnappyCompression) {
    return snappy::Compress(in.data(), in.size(), &out);
  }
#endif
#ifdef LSMDB_HAVE_ZSTD
  if (type == kZstdCompression) {
    size_t bound = ZSTD_compressBound(in.size());
    out.resize(bound);
    size_t rc = ZSTD_compress(&out[0], bound, in.data(), in.size(), 1);
...
#endif  // defined(LSMDB_HAVE_SNAPPY) || defined(LSMDB_HAVE_ZSTD)
