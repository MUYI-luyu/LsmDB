// Copyright (c) 2011 The LsmDB Authors. All rights reserved.
...
class DBTest : public testing::Test {
 public:
  ...
  DBTest() : env_(new SpecialEnv(Env::Default())), option_config_(kDefault) {
    filter_policy_ = new FilterPolicy(10);
  ...
  // test.  Return false there are no more configurations to test.
  bool ChangeOptions() {
    option_config_++;
  ...
  Status TryReopen(Options* options) {
    ...
    Status st = DB::Open(opts, dbname_, &db_);
    if (!st.ok()) {
      std::fprintf(stderr, "*** TryReopen FAILED: %s\n", st.ToString().c_str());
    }
    return st;
  }
  ...
  int NumTableFilesAtLevel(int level) {
    ...
    EXPECT_TRUE(db_->GetProperty(
        "lsmdb.num-files-at-level" + NumberToString(level), &property));
  ...
  std::string DumpSSTableList() {
    std::string property;
    db_->GetProperty("lsmdb.sstables", &property);
    return property;
  }
...
TEST_F(DBTest, GetMemUsage) {
  do {
    ASSERT_LEVELDB_OK(Put("foo", "v1"));
    std::string val;
    ASSERT_TRUE(db_->GetProperty("lsmdb.approximate-memory-usage", &val));
...
TEST_F(DBTest, GetEncountersEmptyLevel) {
  do {
    ...
    // Step 2: clear level 1 necessary.
...
// the database is shutdown during the memtable compaction.
...
TEST_F(DBTest, RecoverWithLargeLog) {
  ...
  // Make sure that we re-open with a small write buffer size that
...
TEST_F(DBTest, ComparatorCheck) {
  class NewComparator : public Comparator {
   public:
    const char* Name() const override { return "lsmdb.NewComparator"; }
...
TEST_F(DBTest, DestroyEmptyDir) {
  ...
  // Should also be destroyed Env is filtering out dot files.
...
TEST_F(DBTest, ManifestWriteError) {
  ...
  // (b) Log record containing F is written to catalog file, but Sync() fails
  // (c) GC deletes F
  // (d) After reopening DB, reads fail since deleted F is named in log record

  // We iterate twice.  In the second iteration, everything is the
  // same except the log record never makes it to the catalog file.
...
TEST_F(DBTest, BloomFilter) {
  ...
  FilterPolicy* fp = new FilterPolicy(10);
  options.filter_policy = fp;
  Reopen(&options);
