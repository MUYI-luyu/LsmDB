...
enum Tag {
  ...
  kLogNumber = 2,       // 当前日志文件编号
  kNextFileNumber = 3,  // 下一个可分配的文件编号
  kLastSequence = 4,    // 最新已分配的序列号
  kCompactPointer = 5,  // Compaction 进度指针
  kDeletedFile = 6,     // 被删除的 SST 文件
  kNewFile = 7,         // 新生成的 SST 文件
...
void VersionDelta::EncodeTo(std::string* dst) const {
  ...
  for (const auto& deleted_file : deleted_files_) {
    ...
    PutVarint32(dst, deleted_file.first);   // level
    PutVarint64(dst, deleted_file.second);  // file number
  }
